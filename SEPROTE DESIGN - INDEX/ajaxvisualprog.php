...
if(isset($_POST["id_prog"])){
   ...
   foreach ($tab_module as $key => $tableau) {
      ...
      /** HEURE TD **/
      $req_td = $bdd->prepare("SELECT SUM(td) as somme FROM volume_horaire WHERE id_module = :id_module");
      $req_td->execute(array("id_module" => $tableau["id_m"]));
      $somme = $req_td->fetch();
      $diff = ($tableau["td_g"] - $somme["somme"]);
      $tab_module[$key]['td_restant'] = $diff;

      /** HEURE TP **/
      $req_tp = $bdd->prepare("SELECT SUM(tp) as somme FROM volume_horaire WHERE id_module = :id_module");
      $req_tp->execute(array("id_module" => $tableau["id_m"]));
      $somme = $req_tp->fetch();
      $diff = ($tableau["tp_g"] - $somme["somme"]);
      $tab_module[$key]['tp_restant'] = $diff;
      ...
      if($tab_per["id_s"] == $tab_sem["id_s"]){
         ...
         foreach ($tab_module as $num_mod => $tab_mod) {
            if($tab_mod["id_p"] == $tab_per["id_p"]){
               $programme .= "<tr class='visualValues'><td>". $tab_mod["code_m"] ."</td><td>". $tab_mod["nom_m"] ."</td><td>". $tab_mod["cm_restant"] ."</td><td>".$tab_mod["td_restant"]."</td><td>".$tab_mod["tp_restant"]."</td></tr>";
            }
         }
      }
   }
