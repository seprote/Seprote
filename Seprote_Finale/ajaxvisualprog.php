<?php
    ...
    $programme = "<table border=1>";
    
    foreach ($tab_semestre as $num_sem => $tab_sem) {
    	$programme .= "<tr><th> Semestre n° ". $num_sem . "(" . $tab_sem["dat_deb_s"]. " au ". $tab_sem["dat_fin_s"] . ")" ."</th></tr>";
    	foreach ($tab_periode as $num_per => $tab_per) {
    		if($tab_per["id_s"] == $tab_sem["id_s"]){

    			$programme .= "<tr><th> Periode n° ". $num_per . "(" . $tab_per["dat_deb_p"]. " au ". $tab_per["dat_fin_p"] . ")" ."</th></tr>";
    			$programme .= "<tr><th>Code module</th><th>Nom module</th><th>H. CM restant</th><th>H. TD restant</th><th>H. TP restant</th></tr>";

    			foreach ($tab_module as $num_mod => $tab_mod) {
    				if($tab_mod["id_p"] == $tab_per["id_p"]){

    					$programme .= "<tr><td>". $tab_mod["code_m"] ."</td><td>". $tab_mod["nom_m"] ."</td>". "<td>". $tab_mod["cm_restant"] ."</td>"."<td></td><td></td></tr>";
    				}
    			}
    		}
    	}
