    ...
    foreach ($tab_semestre as $num_sem => $tab_sem) {
    	...
    	foreach ($tab_periode as $num_per => $tab_per) {
    		if($tab_per["id_s"] == $tab_sem["id_s"]){
    			...
    			$programme .= "<tr><th>Code module</th><th>Nom module</th><th>H. CM</th><th>H. TD</th><th>H. TP</th></tr>";

    			foreach ($tab_module as $num_mod => $tab_mod) {
    				if($tab_mod["id_p"] == $tab_per["id_p"]){

    					$programme .= "<tr><td>". $tab_mod["code_m"] ."</td><td>". $tab_mod["nom_m"] ."</td>". "<td>". $tab_mod["cm_g"] ."</td>"."<td>". $tab_mod["td_g"] ."</td><td>". $tab_mod["tp_g"] ."</td></tr>";
    				}
    			}
    		}
    	}
